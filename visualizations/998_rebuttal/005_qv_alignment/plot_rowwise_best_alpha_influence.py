"""Render row-wise leave-one-task robustness for best-alpha test gain."""
...
def _series(
    records: List[Mapping[str, Any]], task_key: str
) -> Tuple[List[str], np.ndarray, List[str]]:
    if len(records) != 22:
        raise ValueError(
            f"row-wise best-alpha influence must contain 22 records, got {len(records)}"
        )
    tasks = []
    values = []
    undefined = []
    ...
    if len(set(tasks)) != len(tasks):
        raise ValueError(
            f"duplicate tasks in row-wise best-alpha influence field {task_key}"
        )
...
def main() -> None:
    ...
    try:
        ...
        raise ValueError(
            "row-wise best-alpha comparison has incomplete influence statistics"
        ) from error
    if full is None:
        raise ValueError("row-wise best-alpha full-matrix Spearman is undefined")
    ...
    for ax, (field, task_key, title) in zip(axes, panels):
        ...
        if not isinstance(records, list):
            raise ValueError(f"row-wise best-alpha influence lacks {field}")
    ...
    fig.suptitle(
        "Influence sensitivity of mean row cosine versus validation-selected test gain\n"
    ...
    outputs = save_figure(
        fig,
        output_dir,
        "rowwise_best_alpha_alignment_influence",
    )
    plt.close(fig)
