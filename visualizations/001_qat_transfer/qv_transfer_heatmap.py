"""001 — QV Transfer Heatmap (difference vs FP+PTQ)

Loads QV-transfer results and the five baselines (pretrained, fp, fp_ptq, qat,
qat_ptq) for all (target_dataset x qv_dataset) pairs and produces a heatmap
where:
...
Left-panel cell value =
    test_accuracy_patched_qat_ptq[target,qv] - test_accuracy[fp_ptq,target]
...
# project root via `uv run python visualizations/.../qv_transfer_heatmap.py`.
# ---------------------------------------------------------------------------
...
# Constants
# ---------------------------------------------------------------------------
EVAL_ROOT_BASELINES = "evaluations/000_baselines/vision"
EVAL_ROOT_QV        = "evaluations/001_qat_transfer/vision/qv_transfer"
...
def _optim_frag(optim, lr, wd, ls, wl, mgn, bs):
    # NOTE: experiments/* hardcode "optim=adamw" in the path even though they
