...
def validate_grids(
    ...
    if len(ptq) != expected_cells or len(awq) != expected_cells:
        raise ValueError(
            f"expected {expected_cells} cells per method, found PTQ={len(ptq)}, AWQ={len(awq)}"
        )
    if set(ptq) != set(awq):
        missing_ptq = sorted(set(awq) - set(ptq))
        missing_awq = sorted(set(ptq) - set(awq))
        raise ValueError(
            f"grid keys differ; missing PTQ={missing_ptq[:5]}, missing AWQ={missing_awq[:5]}"
        )
...
def matrices(
    ...
    awq_values = np.asarray(
        [
            [float(awq[(source, target)][0][awq_metric]) for source in datasets]
            for target in datasets
        ]
    )
...
def main() -> None:
    ...
    datasets = validate_grids(ptq, awq, args)
    output_dir = output_directory(args)
    output_dir.mkdir(parents=True, exist_ok=True)
    heads: dict[str, Any] = {}
    for head in HEADS:
        ptq_values, awq_values, delta_pp = matrices(head, datasets, ptq, awq, args.split)
        heads[head] = {
        ...
        plot_head(head, datasets, ptq_values, awq_values, delta_pp, args, output_dir)
    metadata = {
