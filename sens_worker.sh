...
Q=/home/lzhou/qat-transfer/sens_jobs.tsv
LK=$LOG/.sens.lock
mkdir -p "$LOG"
export CUDA_VISIBLE_DEVICES=$PHYS
set -a; . ./.env; set +a
# separate output root: these runs record allocations only, and would otherwise
# overwrite the bake-off results at the identical config path
export CHECKPOINT_BASE_PATH=/data02/users/lzhou/qat-transfer/sens_only
...
while :; do
  ...
  ./.venv/bin/python code/experiments/text/sentence_transformers/004_input_fragility/gap_allocation.py \
    ...
    n_random="$NR" sens_only=true > "$LOG/$TAG.log" 2>&1
  rc=$?; d=$(( (SECONDS-t0)/60 ))
