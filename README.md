...
All five scripts live in [code/experiments/vision/000_baselines/](code/experiments/vision/000_baselines/) with matching configs in [config/experiments/vision/000_baselines/](config/experiments/vision/000_baselines/).

### evaluate_pretrained — [code/experiments/vision/000_baselines/evaluate_pretrained.py](code/experiments/vision/000_baselines/evaluate_pretrained.py)
Config: [config/experiments/vision/000_baselines/evaluate_pretrained.yaml](config/experiments/vision/000_baselines/evaluate_pretrained.yaml)

Single local run:
```
uv run --active python code/experiments/vision/000_baselines/evaluate_pretrained.py model_name=openai/clip-vit-base-patch16 dataset_name=CIFAR10 batch_size=128 seed=2038
```

Local sequential sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_pretrained.py -m model_name=openai/clip-vit-base-patch16 dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 seed=1,2,3
```

Submitit parallel sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_pretrained.py -m hydra/launcher=submitit_slurm model_name=openai/clip-vit-base-patch16 dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 seed=1,2,3
```

### evaluate_fp — [code/experiments/vision/000_baselines/evaluate_fp.py](code/experiments/vision/000_baselines/evaluate_fp.py)
Config: [config/experiments/vision/000_baselines/evaluate_fp.yaml](config/experiments/vision/000_baselines/evaluate_fp.yaml)

Single local run:
```
uv run --active python code/experiments/vision/000_baselines/evaluate_fp.py model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=CIFAR10 batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=2038
```

Local sequential sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_fp.py -m model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3
```

Submitit parallel sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_fp.py -m hydra/launcher=submitit_slurm model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3
```

### evaluate_fp_ptq — [code/experiments/vision/000_baselines/evaluate_fp_ptq.py](code/experiments/vision/000_baselines/evaluate_fp_ptq.py)
Config: [config/experiments/vision/000_baselines/evaluate_fp_ptq.yaml](config/experiments/vision/000_baselines/evaluate_fp_ptq.yaml). Adds a `ptq` group.

Single local run:
```
uv run --active python code/experiments/vision/000_baselines/evaluate_fp_ptq.py model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=CIFAR10 batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=2038 ptq.bits=3 ptq.granularity=channel 'ptq.skip_modules=[classification_head]'
```

Local sequential sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_fp_ptq.py -m model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3 ptq.bits=2,4,8 ptq.granularity=tensor,channel 'ptq.skip_modules=[classification_head]'
```
...
### evaluate_qat — [code/experiments/vision/000_baselines/evaluate_qat.py](code/experiments/vision/000_baselines/evaluate_qat.py)
Config: [config/experiments/vision/000_baselines/evaluate_qat.yaml](config/experiments/vision/000_baselines/evaluate_qat.yaml). The `qat.*` group must match the training config of the checkpoint being evaluated.

Single local run:
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat.py model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=CIFAR10 batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=2038 qat.bits=3 qat.granularity=channel 'qat.skip_modules=[classification_head]'
```

Local sequential sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat.py -m model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3 qat.bits=2,4,8 qat.granularity=tensor,channel 'qat.skip_modules=[classification_head]'
```

Submitit parallel sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat.py -m hydra/launcher=submitit_slurm model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3 qat.bits=2,4,8 qat.granularity=tensor,channel 'qat.skip_modules=[classification_head]'
```

### evaluate_qat_ptq — [code/experiments/vision/000_baselines/evaluate_qat_ptq.py](code/experiments/vision/000_baselines/evaluate_qat_ptq.py)
Config: [config/experiments/vision/000_baselines/evaluate_qat_ptq.yaml](config/experiments/vision/000_baselines/evaluate_qat_ptq.yaml). Has both a `qat` group (to locate the checkpoint) and a `ptq` group (for the PTQ applied at eval time).

Single local run:
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat_ptq.py model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=CIFAR10 batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=2038 qat.bits=3 qat.granularity=channel 'qat.skip_modules=[classification_head]' ptq.bits=3 ptq.granularity=channel 'ptq.skip_modules=[classification_head]'
```

Local sequential sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat_ptq.py -m model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3 qat.bits=3 qat.granularity=channel 'qat.skip_modules=[classification_head]' ptq.bits=2,4,8 ptq.granularity=tensor,channel 'ptq.skip_modules=[classification_head]'
```

Submitit parallel sweep (all datasets):
```
uv run --active python code/experiments/vision/000_baselines/evaluate_qat_ptq.py -m hydra/launcher=submitit_slurm model_name=openai/clip-vit-base-patch16 pretrained=true dataset_name=Cars,DTD,EuroSAT,GTSRB,MNIST,RESISC45,SUN397,SVHN,CIFAR10,CIFAR100,STL10,Food101,Flowers102,FER2013,PCAM,OxfordIIITPet,RenderedSST2,EMNIST,FashionMNIST,KMNIST,TinyImageNet,ImageNet batch_size=128 lr=1e-5 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 seed=1,2,3 qat.bits=3 qat.granularity=channel 'qat.skip_modules=[classification_head]' ptq.bits=2,4,8 ptq.granularity=tensor,channel 'ptq.skip_modules=[classification_head]'
```
...
### qv_transfer — [code/experiments/vision/001_qat_transfer/qv_transfer.py](code/experiments/vision/001_qat_transfer/qv_transfer.py)
Config: [config/experiments/vision/001_qat_transfer/qv_transfer.yaml](config/experiments/vision/001_qat_transfer/qv_transfer.yaml)
