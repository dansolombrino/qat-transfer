...
# stage: compute_rowwise_alignment
# wave: 20260803-120627   rig: rig-4090   gpu lane identity: 0 (CPU-only run)
set -uo pipefail
cd "$(dirname "$0")/../../../../.." || exit 1

RUN_FOLDER="producer_run_id_sha256=8ca847e02a7acfc937aa9e0fb5b5bd593254c92e0ae95e764a3c4d61299332b2"
...
EVAL_DIR="evaluations/998_rebuttal/005_qv_alignment/rowwise_alignment/family=ilharco_timm_supervised/model_name=vit_base_patch16_224.orig_in21k/seed=2038/optim=adamw/lr=1e-05/wd=0.1/ls=0.0/wl=500/max_grad_norm=1.0/batch_size=128/qat_bits=3/qat_granularity=channel/qat_skip_modules=%5B%22head%22%5D/ptq_skip_modules=%5B%22head%22%5D/checkpoint_kind=classifier/epoch_policy=dataset_final/vector_scope=quantized_linear_weight/module_selector=apply_ptq_linear_v1/accumulation_dtype=float64/aggregation_spec=row_cosine_mean_v1"
LOG_DIR="logs/998_rebuttal/005_qv_alignment/$RUN_FOLDER/wave_20260803-120627"
ARTIFACT="$EVAL_DIR/rowwise_alignment.json"
...
.venv/bin/python \
  code/experiments/998_rebuttal/005_qv_alignment/compute_rowwise_alignment.py \
  "${HYDRA_ARGS[@]}" 2>&1 \
