#!/usr/bin/env bash
# run: model=bert-large,split=val,alpha=-0.1,receiver=Emotion,donors=all   experiment: 998_rebuttal/003_lambda_sensitivity/001_signed_bert
# wave: 20260802-152022   rig: behemoth   gpu: 2
# GPU auth: user-granted 2026-08-02 for wave 20260802-152022: gpu 0,2,4,5,6,7
set -uo pipefail
cd "$(dirname "$0")/../../../../../.." || exit 1

RUN_ID_FLAT=model=bert-large,split=val,alpha=-0.1,receiver=Emotion,donors=all
EVAL_DIR=evaluations/998_rebuttal/003_lambda_sensitivity/001_signed_bert/model=bert-large/split=val/alpha=-0.1/receiver=Emotion/donors=all
LOG_DIR=logs/998_rebuttal/003_lambda_sensitivity/001_signed_bert/model=bert-large,split=val,alpha=-0.1,receiver=Emotion,donors=all/wave_20260802-152022
ARTIFACT=evaluations/998_rebuttal/003_lambda_sensitivity/001_signed_bert/model=bert-large/split=val/alpha=-0.1/receiver=Emotion/donors=all/complete.json
...
.venv/bin/python code/experiments/998_rebuttal/003_lambda_sensitivity/001_signed_bert/run_row.py model=bert-large split=val alpha=-0.1 receiver=Emotion donors=all 2>&1   | tee "$LOG_DIR/wave_behemoth_gpu2-$(date +%Y%m%d-%H%M%S).log"
