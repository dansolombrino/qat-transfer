...
.venv/bin/python code/experiments/vision/ilharco_timm_supervised/009_qat_transfer_awq/qv_transfer_awq.py model_name=vit_base_patch16_224.orig_in21k batch_size=128 lr=1e-05 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 gpu=0 eval_split=test limit_num_batches=null log_to_file=false skip_existing=true 'source.dataset_names=["Flowers102"]' source.seed=2038 'target.dataset_names=["CIFAR100"]' target.seed=2038 qat.bits=3 qat.granularity=channel 'qat.skip_modules=["head"]' 'qv.alphas=[1.0]' awq.bits=3 awq.granularity=channel 'awq.skip_modules=["head"]' awq.num_calib_batches=4 awq.n_grid=20 awq.clip=true 2>&1 \
