#!/usr/bin/env bash
# run: method=gptq,src=ImageNet,tgt=SVHN,qv=agrid11,split=val   wave: 20260802-212527   rig: behemoth   gpu: 7
set -uo pipefail
...
if "$all_done"; then
  echo "[skip] all 11 validation artifacts already exist for gptq/SVHN"
  exit 0
fi
...
LOG_DIR='logs/vision/reviewer_3hfp/imagenet_strong_ptq_alpha_sweep/method=gptq,src=ImageNet,tgt=SVHN,qv=agrid11,split=val/wave_20260802-212527'
mkdir -p "$LOG_DIR" || exit 1

.venv/bin/python code/experiments/vision/ilharco_timm_supervised/005_qat_transfer_gptq/qv_transfer_gptq.py model_name=vit_base_patch16_224.orig_in21k batch_size=128 lr=1e-05 wd=0.1 ls=0.0 wl=500 max_grad_norm=1.0 gpu=0 eval_split=val 'source.dataset_names=["ImageNet"]' source.seed=2038 'target.dataset_names=["SVHN"]' target.seed=2038 qat.bits=3 qat.granularity=channel 'qat.skip_modules=["head"]' 'qv.alphas=[0.15,0.3,0.45,0.6,0.75,0.9,1.0,1.05,1.2,1.35,1.5]' gptq.bits=3 gptq.granularity=channel 'gptq.skip_modules=["head"]' gptq.num_calib_batches=4 gptq.percdamp=0.01 gptq.actorder=false gptq.block_size=128 2>&1 \
