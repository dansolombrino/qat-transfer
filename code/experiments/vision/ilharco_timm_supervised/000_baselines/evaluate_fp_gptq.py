...
from src.vision.ilharco_timm_supervised.baselines.gptq import apply_gptq_
...
def main(cfg: DictConfig):
    ...
    quantized_names = apply_gptq_(
        ...
        calibration_loader=dataset.val_loader,
        num_calibration_samples=cfg.gptq.num_calibration_samples,
    )

    skipped_names = sorted(set(all_linear_names) - set(quantized_names))

    print(f"\nGPTQ config: bits={cfg.gptq.bits}, granularity={cfg.gptq.granularity}, sym={cfg.gptq.sym}, skip_modules={list(cfg.gptq.skip_modules)}, num_calibration_samples={cfg.gptq.num_calibration_samples}")
    ...
    eval_dir = os.path.join(
        ...
        cfg.dataset_name,
        f"optim=adamw_lr={cfg.lr}_wd={cfg.wd}_ls={cfg.ls}_wl={cfg.wl}_mgn={cfg.max_grad_norm}_bs={cfg.batch_size}",
        f"gptq=bits={cfg.gptq.bits}_gran={cfg.gptq.granularity}_sym={cfg.gptq.sym}_skip={skip_modules_tag}_ncal={cfg.gptq.num_calibration_samples}",
        f"seed={cfg.seed}",
    )
    ...
    results = {
        ...
        "gptq_sym": cfg.gptq.sym,
        "gptq_skip_modules": list(cfg.gptq.skip_modules),
        "gptq_num_calibration_samples": cfg.gptq.num_calibration_samples,
