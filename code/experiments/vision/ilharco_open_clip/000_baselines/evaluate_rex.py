...
from src.rex import apply_rex_ptq_terms_open_clip_
...
def main(cfg: DictConfig):
    ...
    skip_modules = frozenset(cfg.rex.skip_modules)
    ...
    quantized_names = apply_rex_ptq_terms_open_clip_(
        model=image_classifier,
        bits=cfg.rex.bits,
        granularity=cfg.rex.granularity,
        order=cfg.rex.order,
        sparsity=cfg.rex.sparsity,
        skip_modules=skip_modules,
    )

    skipped_names = sorted(
        (set(all_linear_names) | set(all_mha_names)) - set(quantized_names)
    )

    print(f"\nREx config: bits={cfg.rex.bits}, granularity={cfg.rex.granularity}, order={cfg.rex.order}, sparsity={cfg.rex.sparsity}, skip_modules={list(cfg.rex.skip_modules)}")
    ...
    print(f"\n    eval test_accuracy (REx PTQ): {test_accuracy}\n")
    ...
    skip_modules_tag = "-".join(sorted(cfg.rex.skip_modules)) if len(cfg.rex.skip_modules) > 0 else "none"
    ...
    eval_dir = os.path.join(
        ...
        cfg.dataset_name,
        f"optim=adamw_lr={cfg.lr}_wd={cfg.wd}_ls={cfg.ls}_wl={cfg.wl}_mgn={cfg.max_grad_norm}_bs={cfg.batch_size}",
        f"rex=bits={cfg.rex.bits}_order={cfg.rex.order}_sparsity={cfg.rex.sparsity}_gran={cfg.rex.granularity}_skip={skip_modules_tag}",
        f"seed={cfg.seed}",
    )
    ...
    results = {
        ...
        "rex_bits": cfg.rex.bits,
        "rex_granularity": cfg.rex.granularity,
        "rex_order": cfg.rex.order,
        "rex_sparsity": cfg.rex.sparsity,
        "rex_skip_modules": list(cfg.rex.skip_modules),
