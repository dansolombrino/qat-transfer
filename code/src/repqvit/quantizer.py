...
class UniformQuantizer(nn.Module):
    """
    Asymmetric uniform quantization (affine quantization).
    Initializes scale and zero_point from data on first forward pass.
    Supports tensor-wise and channel-wise modes.
    """
    ...
    def __repr__(self):
        s = super().__repr__()
        return "(" + s + f" inited={self.inited}, channel_wise={self.channel_wise})"
...
class LogSqrt2Quantizer(nn.Module):
    """
    Log-sqrt(2) quantizer for post-softmax activations.
    The attention map values are non-negative after softmax and follow a
    heavy-tailed distribution that log quantization handles better.
    """
    def __init__(self, n_bits: int = 8, channel_wise: bool = False):
        super().__init__()
        assert 2 <= n_bits <= 8, 'bitwidth not supported'
