...
from ..utils import SPLIT_SEED, sanitize_open_clip_model_name
...
def build_classification_head(model, tokenizer, dataset_name, device):
    ...
    # threaded in here: a single cached head per (model_name, pretrained,
    # dataset_name) is correct for *every* run seed of finetune_fp /
    # evaluate_*, and regenerating it per seed would produce a bit-identical
    # file.
    dataset = get_dataset(
        dataset_name,
...
def get_classification_head(model_name: str, pretrained: str, dataset_name: str, save_dir: str, device):
    filename = os.path.join(save_dir, "vision", "ilharco_open_clip", sanitize_open_clip_model_name(model_name, pretrained), f"head_{dataset_name}.pt")
    if os.path.exists(filename):
        print(f"Classification head for {model_name}/{pretrained} on {dataset_name} exists at {filename}")
        return ClassificationHead.load(filename)

    print(
        f"Did not find classification head for {model_name}/{pretrained} on {dataset_name} at {filename}, "
        f"building one from scratch."
    )
    encoder = ImageEncoder(model_name=model_name, pretrained=pretrained, keep_lang=True)
    model = encoder.model
