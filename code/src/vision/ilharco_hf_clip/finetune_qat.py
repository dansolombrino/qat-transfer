...
def _evaluate_ptq(
    ...
    plain nn.Module subclasses wrapping a transformers.CLIPModel + an
    nn.Linear-based head. No forward/backward hooks or non-picklable attrs
    are registered. QATLinear stores only a child nn.Linear plus Python
...
def main(cfg: DictConfig) -> None:
    ...
    encoder = ImageEncoder(model_name=cfg.model_name, keep_lang=False)
    ...
    # This family performs one optimizer step per batch -- there is no gradient
    # accumulation here -- so batches and optimizer steps coincide and
    ...
    #
    # This also repairs a pre-existing dryrun inconsistency: the loop ran
