"""PTQ4ViT quantized linear layers.

Copied from references/ptq4vit/code/quant_layers/linear.py with:
- Import path fixes (removed unused cross-module import)
...
class PTQSLQuantLinear(MinMaxQuantLinear):
    ...
    def _search_best_w_interval(self, x, weight_interval_candidates, raw_out_expanded_chunked):
        ...
        for h in range(self.n_H):
            ...
            for p_st in range(0,self.eq_n,self.parallel_eq_n):
                ...
                similarity = self._get_similarity(raw_out_expanded_chunked, out_sim, self.metric)
                similarity = torch.mean(similarity, dim=list(range(len(similarity.shape)-2)))
    ...
    def _search_best_a_interval(self, x, input_interval_candidates, raw_out_expanded):
        ...
        for a in range(self.n_a):
            ...
            for p_st in range(0,self.eq_n,self.parallel_eq_n):
                ...
                x_sim = x_sim.permute(*list(range(len(x_sim.shape)-3)),-1,-3,-2).reshape(*x.shape[:-1],p_ed-p_st,x.shape[-1])
                out_sim = F.linear(x_sim, w_sim, bias_sim)
                similarity = self._get_similarity(raw_out_expanded, out_sim, self.metric)
                similarity = torch.mean(similarity, dim=list(range(len(similarity.shape)-1)))
                similarities.append(similarity)
            similarities = torch.cat(similarities, dim=0)
    ...
    def calibration_step2(self,x):
        ...
        raw_out_expanded = self.raw_out.to(device).unsqueeze(-2)
        raw_out_expanded_chunked = torch.cat(torch.chunk(raw_out_expanded.unsqueeze(-2), chunks=self.n_V, dim=-1), dim=-2)

        self.raw_grad = self.raw_grad.to(device) if self.raw_grad is not None else None

        weight_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(device).view(-1,1,1,1,1) * self.w_interval.unsqueeze(0)
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(device).view(1,1,-1) * self.a_interval.unsqueeze(-1)
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for h in range(self.n_H):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                raw_out_expanded = self.raw_out[b_st:b_ed].to(self._device).unsqueeze(-2)
    ...
    def calibration_step2(self):
        ...
        input_interval_candidates =  torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(self._device).view(1,1,-1) * self.a_interval.unsqueeze(-1)
        for e in range(self.search_round):
        ...
        for a in range(self.n_a):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0,self.eq_n,self.parallel_eq_n):
                    ...
                    x_sim = (x_pos + x_neg).permute(*list(range(len(x_sim.shape)-3)),-1,-3,-2).reshape(*x.shape[:-1],p_ed-p_st,x.shape[-1])
                    out_sim = F.linear(x_sim, w_sim, bias_sim)
                    similarity = self._get_similarity(raw_out_expanded, out_sim, self.metric, raw_grad)
                    similarity = torch.mean(similarity, dim=list(range(1,len(similarity.shape)-1)))
                    similarity = torch.sum(similarity, dim=0, keepdim=True)
