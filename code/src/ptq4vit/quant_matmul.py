"""PTQ4ViT quantized matmul layers.

Copied from references/ptq4vit/code/quant_layers/matmul.py with:
- .cuda() replaced with device-aware .to() calls
- MatMul modules store _dev attribute for device tracking
"""
...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def _search_best_B_interval(self, A, B, B_interval_candidates):
        ...
        for v, h in product(range(self.n_V_B), range(self.n_H_B)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                B_sim = B_sim.view(p_ed-p_st,-1,B.shape[1]+self.pad_groups_B,B.shape[2]+self.pad_rows_B,B.shape[3]+self.pad_cols_B)
                ...
                similarities.append(similarity)
            similarities = torch.cat(similarities, 0)
            similarities = F.pad(similarities, [0,self.pad_groups_B]).view(self.eq_n,self.n_G_B,self.crb_groups_B).mean(-1)
            best_index = torch.argmax(similarities, dim=0, keepdim=False).view(1,1,-1,1,1,1,1,1)
    ...
    def _initialize_intervals(self, A, B):
        ...
        if self.init_layerwise:
            ...
            self.A_interval=(A_pad.abs().amax([0,1,3,5,7], keepdim=True)/(self.A_qmax-0.5)).detach().squeeze(0)
            self.B_interval=(B_pad.abs().amax([0,1,3,5,7], keepdim=True)/(self.B_qmax-0.5)).detach().squeeze(0)

    def calibration_step2(self, A, B):
        self.raw_out = self.raw_out.unsqueeze(0).to(A.device)
        self.raw_grad = self.raw_grad.to(A.device) if self.raw_grad is not None else None
...
class PTQSLBatchingQuantMatMul(PTQSLQuantMatMul):
    ...
    def _initialize_calib_parameters(self):
        ...
        while True:
            numel = ((self.raw_input[0].numel()+self.raw_input[1].numel()+2*self.raw_out.numel())/self.calib_size*self.calib_batch_size)
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0,self.calib_size,self.calib_batch_size):
            ...
            A, B = self.raw_input[0][b_st:b_ed].to(self._dev), self.raw_input[1][b_st:b_ed].to(self._dev)
            if self.init_layerwise:
                ...
                A_pad = F.pad(A, [0,self.pad_cols_A,0,self.pad_rows_A,0,self.pad_groups_A]).unsqueeze(0).view(1,-1,self.n_G_A,self.crb_groups_A,self.n_V_A,self.crb_rows_A,self.n_H_A,self.crb_cols_A)
                B_pad = F.pad(B, [0,self.pad_cols_B,0,self.pad_rows_B,0,self.pad_groups_B]).unsqueeze(0).view(1,-1,self.n_G_B,self.crb_groups_B,self.n_V_B,self.crb_rows_B,self.n_H_B,self.crb_cols_B)
                A_interval=(A_pad.abs().amax([0,1,3,5,7], keepdim=True)/(self.A_qmax-0.5)).detach().squeeze(0)
                B_interval=(B_pad.abs().amax([0,1,3,5,7], keepdim=True)/(self.B_qmax-0.5)).detach().squeeze(0)
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None, dim=-1, raw_grad=None):
        if metric == "cosine":
            ...
            similarity = F.cosine_similarity(tensor_raw-torch.mean(tensor_raw,dim=dim,keepdim=True), tensor_sim-torch.mean(tensor_sim,dim=dim,keepdim=True), dim=dim)
        else:
            if metric == "L1_norm":
                similarity = -torch.abs(tensor_raw - tensor_sim)
            elif metric == "L2_norm":
    ...
    def _search_best_A_interval(self, A_interval_candidates):
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
                A = self.raw_input[0][b_st:b_ed].to(self._dev)
                A_pad = F.pad(A, [0,self.pad_cols_A,0,self.pad_rows_A,0,self.pad_groups_A]).unsqueeze(0).view(1,-1,self.n_G_A,self.crb_groups_A,self.n_V_A,self.crb_rows_A,self.n_H_A,self.crb_cols_A)
                B = self.raw_input[1][b_st:b_ed].to(self._dev)
                B_sim = self.quant_input_B(B).unsqueeze(0)
                raw_out = self.raw_out[b_st:b_ed].unsqueeze(0).to(self._dev)
                raw_grad = self.raw_grad[b_st:b_ed].to(self._dev)
                similarities = []
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    p_ed = min(self.eq_n,p_st+self.parallel_eq_n)
                    cur_A_interval = tmp_A_interval.repeat(p_ed-p_st,1,1,1,1,1,1,1)
            ...
            batch_similarities = F.pad(batch_similarities, [0,self.pad_groups_A]).view(self.eq_n,self.n_G_A,self.crb_groups_A).mean(-1)
    ...
    def _search_best_B_interval(self, B_interval_candidates):
        ...
        for v, h in product(range(self.n_V_B), range(self.n_H_B)):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                batch_similarities.append(similarities)
            batch_similarities = torch.cat(batch_similarities, dim=1).sum(dim=1, keepdim=False)
            batch_similarities = F.pad(batch_similarities, [0,self.pad_groups_B]).view(self.eq_n,self.n_G_B,self.crb_groups_B).mean(-1)
            best_index = torch.argmax(batch_similarities, dim=0, keepdim=False).view(1,1,-1,1,1,1,1,1)
            tmp_B_interval[:,:,:,:,v:v+1,:,h:h+1,:] = torch.gather(B_interval_candidates[:,:,:,:,v:v+1,:,h:h+1,:],dim=0,index=best_index)
        self.B_interval = tmp_B_interval.squeeze(0)
    ...
    def calibration_step2(self):
        ...
        A_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(self._dev).view(-1,1,1,1,1,1,1,1) * self.A_interval.unsqueeze(0)
        B_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(self._dev).view(-1,1,1,1,1,1,1,1) * self.B_interval.unsqueeze(0)
        for e in range(self.search_round):
...
class SoSPTQSLBatchingQuantMatMul(PTQSLBatchingQuantMatMul):
    ...
    def _search_best_A_interval(self, split_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            b_ed = min(self.calib_size, b_st + self.calib_batch_size)
            A = self.raw_input[0][b_st:b_ed].unsqueeze(0).to(self._dev)
            B = self.raw_input[1][b_st:b_ed].unsqueeze(0).to(self._dev)
            B_sim = B
            raw_out = self.raw_out[b_st:b_ed].unsqueeze(0).to(self._dev)
            raw_grad = self.raw_grad[b_st:b_ed].to(self._dev)
            similarities = []
            for i in range(len(split_candidates)):
                ...
                out_sim = A_sim @ B_sim
                similarity = self._get_similarity(raw_out, out_sim, self.metric, raw_grad=raw_grad)
                similarity = similarity.mean([2,3])
                similarity = similarity.sum(dim=1,keepdim=True)
                similarities.append(similarity)
            similarities = torch.cat(similarities, 0)
            batch_similarities.append(similarities)
    ...
    def calibration_step2(self):
        self._initialize_calib_parameters()
        self._initialize_intervals()
        A_split_candidates = torch.tensor([2**(-i) for i in range(20)]).to(self._dev)
        B_interval_candidates = torch.tensor([self.eq_alpha + i*(self.eq_beta - self.eq_alpha)/self.eq_n for i in range(self.eq_n + 1)]).to(self._dev).view(-1,1,1,1,1,1,1,1) * self.B_interval.unsqueeze(0)
        for e in range(self.search_round):
