...
class MassiveScenario:
    def __init__(self, batch_size, num_workers, seed):

        hf_train = load_dataset("mteb/amazon_massive_scenario", "en", split="train", token=HF_TOKEN, cache_dir=HF_DATASETS_CACHE)
        hf_test = load_dataset("mteb/amazon_massive_scenario", "en", split="test", token=HF_TOKEN, cache_dir=HF_DATASETS_CACHE)
