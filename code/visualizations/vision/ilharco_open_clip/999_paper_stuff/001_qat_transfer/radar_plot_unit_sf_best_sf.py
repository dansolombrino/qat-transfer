"""Radar plot: FT+PTQ vs QV Patching+PTQ (λ=1) vs QV Patching+PTQ (best λ) vs QAT+PTQ
...
def plot_radar(all_data, model_labels, args, optim_frag, qat_frag):
    ...
    out_dir = os.path.join(
        ...
        "radar_plot_unit_sf_best_sf", f"seed={args.seed}", f"smult={mult_tag(args.source_epoch_mult)}", f"tmult={mult_tag(args.target_epoch_mult)}", optim_frag, qat_frag,
        _ptq_frag(args),
    )
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "radar_plot_unit_sf_best_sf.pdf")
