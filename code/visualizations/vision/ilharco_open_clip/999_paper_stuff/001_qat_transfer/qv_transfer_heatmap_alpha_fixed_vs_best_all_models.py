"""999 — QV Transfer Side-by-Side Heatmaps: Fixed Alpha vs Best Alpha — ALL open_clip models

Produces one giant PDF with all 3 open_clip models arranged in a 2-row × 2-column grid:

    Row 0 :  [Model 1 pair]   [Model 2 pair]
    Row 1 :        [Model 3 pair left-aligned]
...
def parse_args():
    ...
    parser.add_argument("--pretrained-tags", required=True, nargs="+",
                        help="One pretrained tag per model (3 values), positionally "
    ...
    parser.add_argument("--batch-size",     required=True, nargs="+", type=int,
                        help="One batch size per model (3 values), positionally "
                             "matched to the hardcoded MODELS list.")
    ...
    assert len(args.pretrained_tags) == len(MODELS), (
        f"Expected {len(MODELS)} pretrained-tags values (one per model), got {len(args.pretrained_tags)}"
    )
    return args
...
# ---------------------------------------------------------------------------
def plot_all_models(all_data_fixed, all_data_best, args):
    n_models = len(MODELS)
    n_rows = (n_models + 1) // 2  # 2 rows for 3 models
    ...
    out_path = os.path.join(out_dir, "heatmap_alpha_fixed_vs_best_all_models.pdf")
...
def main():
    ...
    for model_idx, model_name in enumerate(MODELS):
        pretrained = args.pretrained_tags[model_idx]
        ...
        all_data_fixed[model_name] = load_data_fixed_alpha(
            args, model_dir, optim_frag, qat_frag, ptq_frag,
        )
        all_data_best[model_name] = load_data_best_alpha(
            args, model_dir, optim_frag, qat_frag, ptq_frag,
        )

    plot_all_models(all_data_fixed, all_data_best, args)
