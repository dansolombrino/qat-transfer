...
Produces two difference heatmaps side-by-side:
    ...
    Left  : cell = qv_transfer_ptq(fixed alpha) accuracy - FP+PTQ accuracy
    Right : cell = qv_transfer_ptq(best alpha) accuracy  - FP+PTQ accuracy
...
def plot_sidebyside(data_fixed, data_best, args, model_dir, optim_frag, qat_frag):
    datasets = sorted(data_fixed.keys())
    n = len(datasets)
