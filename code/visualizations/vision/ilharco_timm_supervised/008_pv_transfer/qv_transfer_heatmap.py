...
def _pv_path(model_dir, dataset, seed, optim_frag, pv_frag):
    return os.path.join(
        EVAL_ROOT_BASELINES, "pv", model_dir, dataset,
        optim_frag, pv_frag, f"seed={seed}", "eval_results.json",
    )


def _pv_ptq_path(model_dir, dataset, seed, optim_frag, pv_frag, ptq_frag):
    return os.path.join(
        EVAL_ROOT_BASELINES, "pv_ptq", model_dir, dataset,
        optim_frag, pv_frag, ptq_frag, f"seed={seed}", "eval_results.json",
    )
...
def load_data(args):
    ...
    for target_dataset in datasets:
        data[target_dataset] = {
            ...
            "pv": _load_value(
                _pv_path(model_dir, target_dataset, args.seed, optim_frag, pv_frag),
                TEST_ACC_KEY,
            ),
            "pv_ptq": _load_value(
                _pv_ptq_path(model_dir, target_dataset, args.seed,
                             optim_frag, pv_frag, ptq_frag),
                TEST_ACC_KEY,
            ),
            "random": (
