"""005 — QV Transfer under GPTQ Heatmap — timm supervised

Loads the 005_qat_transfer_gptq results (QV patching with GPTQ as the final
...
random, qat, qat+PTQ) plus GPTQ(FP): the sweep's own alpha=0 self-pair cell,
which is GPTQ applied to the receiver's FP checkpoint under calibration
identical to the alpha=1 cells.

Two difference heatmaps per head variant:
  - minus gptq_fp: the Task-2 delta (does QV still add gain under GPTQ?)
  - minus fp_ptq:  QV+GPTQ against the vanilla-RTN world (the paper's original
...
EVAL_ROOT_QV        = "evaluations/vision/ilharco_timm_supervised/005_qat_transfer_gptq/vision/qv_transfer_gptq"
...
def _qv_metric_keys(eval_split):
    return {
        "fp_head_gptq":  f"{eval_split}_accuracy_fp_head_gptq",
        "qat_head_gptq": f"{eval_split}_accuracy_qat_head_gptq",
    }

QV_METRIC_LABELS = {
    "fp_head_gptq":  "FP Head",
    "qat_head_gptq": "QAT Head",
}
...
def parse_args():
    ...
    # gptq path-fragment components (block_size is deliberately absent: it is
    # result-invariant solver batching and is not part of the eval path)
...
def _qv_transfer_path(model_dir, qv_dataset, target_dataset, seed,
                       optim_frag, qat_frag, gptq_frag, qv_frag, eval_split, *, source_epoch_mult, target_epoch_mult):
    return os.path.join(
        ...
        optim_frag, qat_frag, gptq_frag, qv_frag,
        f"split={eval_split}",
        "eval_results.json",
    )
...
def load_data(args):
    ...
    for target_dataset in datasets:
        data[target_dataset] = {
            ...
            # GPTQ(FP): the sweep's own alpha=0 self-pair cell — GPTQ applied to
            ...
            "gptq_fp": _load_value(
                _qv_transfer_path(
                    ...
                    optim_frag, qat_frag, gptq_frag, _qv_frag(0.0), eval_split,
                
                source_epoch_mult=args.source_epoch_mult, target_epoch_mult=args.target_epoch_mult,
            ),
                f"{eval_split}_accuracy_fp_head_gptq",
            ),
            "random": (
        ...
        for qv_dataset in datasets:
            qv_path = _qv_transfer_path(
                ...
                optim_frag, qat_frag, gptq_frag, qv_frag, eval_split,
    ...
    return data, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag
...
# Plot
# ---------------------------------------------------------------------------
def plot_heatmap(data, args, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag, metric_tag):
    """Raw QV transfer+GPTQ accuracy heatmap (sequential Viridis, cmin=0, cmax=1)."""
    ...
    out_dir = _out_dir(args, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag)
...
def plot_difference_heatmap(data, args, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag,
                            metric_tag, subtractor="gptq_fp"):
    datasets = sorted(data.keys(), key=str.lower)
    head_label = QV_METRIC_LABELS[metric_tag]
...
def main():
    args = parse_args()
    data, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag = load_data(args)
    common = (data, args, model_dir, optim_frag, qat_frag, gptq_frag, qv_frag)
    for metric_tag in _qv_metric_keys(args.eval_split):
        plot_heatmap(*common, metric_tag=metric_tag)
        plot_difference_heatmap(*common, metric_tag=metric_tag, subtractor="gptq_fp")
