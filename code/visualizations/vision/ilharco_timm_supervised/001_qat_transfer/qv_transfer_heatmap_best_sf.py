...
each cell, picks the alpha that achieves the highest accuracy across all alpha
values swept on disk.  Produces one heatmap per head variant (FP head / QAT head):
...
def load_data(args):
    ...
    for target_dataset in datasets:
        ...
        for qv_dataset in datasets:
            ...
            # Track best alpha independently per metric.
            best = {mt: {"acc": None, "val": None} for mt in QV_METRIC_KEYS}
            ...
            for alpha_file in alpha_files:
                ...
                for metric_tag, metric_key in QV_METRIC_KEYS.items():
                    acc = _load_value(alpha_file, metric_key)
                    if acc is None:
                        continue
                    if best[metric_tag]["acc"] is None or acc > best[metric_tag]["acc"]:
                        best[metric_tag]["acc"] = acc
                        best[metric_tag]["val"] = alpha_val
            ...
            data[target_dataset]["qv_transfer"][qv_dataset] = {
                metric_tag: {
                    "best_alpha_acc": best[metric_tag]["acc"],
                    "best_alpha_val": best[metric_tag]["val"],
                }
