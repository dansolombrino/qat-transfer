...
EVAL_ROOT_QV        = "evaluations/vision/ilharco_timm_supervised/002z_qat_transfer_reversed_ptq_after_reverse/vision/qv_transfer"
...
def plot_heatmap(data, args, model_dir, optim_frag, qat_frag, qv_frag, metric_tag):
    ...
    formula = "cell = Acc[ PTQ(PTQ(QAT_tgt) \u2212 \u03b1\u00b7QV) ]"
    title = (
        f"Reversed QV Transfer+PTQ ({head_label})<br>"
    ...
    out_dir = os.path.join(
        "plots", "vision", "ilharco_timm_supervised", "002z_qat_transfer_reversed_ptq_after_reverse", "qv_transfer_heatmap",
...
def plot_difference_heatmap(data, args, model_dir, optim_frag, qat_frag, qv_frag,
    ...
    formula = f"cell = Acc[ PTQ(PTQ(QAT_tgt) \u2212 \u03b1\u00b7QV) ] \u2212 {baseline_formula}"
    title = (
