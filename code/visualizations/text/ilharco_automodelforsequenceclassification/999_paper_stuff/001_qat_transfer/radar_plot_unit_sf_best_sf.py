"""Radar plot: FT+PTQ vs QV Patching+PTQ (λ=1) vs QV Patching+PTQ (best λ) vs QAT+PTQ (text, automodel)

Two or three radar plots side-by-side (one per model scale).
Each radar has 11 axes (one per text dataset) and four webs:
...
def parse_args():
    ...
    parser.add_argument("--model-names",     required=True, nargs="+",
                        help="Two or three HF model names (e.g. "
    ...
    if not 1 <= len(args.model_names) <= 4:
        parser.error("--model-names must have 1 to 4 entries")
    return args
...
def plot_radar(all_data, model_labels, args, metric_tag):
    ...
    skip_tag = "-".join(dict.fromkeys(args.skip_modules))
    qat_frag_out = f"qat=bits={args.qat_bits}_gran={args.granularity}_skip={skip_tag}"
    out_dir = os.path.join(
        "plots", "text", "ilharco_automodelforsequenceclassification",
        "999_paper_stuff", "001_qat_transfer",
        "radar_plot_unit_sf_best_sf", model_tag, f"seed={args.seed}", f"smult={mult_tag(args.source_epoch_mult)}", f"tmult={mult_tag(args.target_epoch_mult)}", qat_frag_out,
