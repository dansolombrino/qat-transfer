"""Mixed-modality QV Transfer Heatmap (best alpha, independent color scales)
...
Each modality has its own independent symmetric color scale and colorbar.
...
def plot_sidebyside(vision_data, text_data, args,
    ...
    # -- independent color bounds ---------------------------------------------
    finite_vis = z_vis[np.isfinite(z_vis)].tolist()
    finite_txt = z_txt[np.isfinite(z_txt)].tolist()
    cmin_vis, cmax_vis = _robust_symmetric_bounds(finite_vis, center=0.0, min_span=0.02)
    cmin_txt, cmax_txt = _robust_symmetric_bounds(finite_txt, center=0.0, min_span=0.02)

    # -- colormaps & norms ----------------------------------------------------
    cmap_div = plt.get_cmap("RdYlGn")
    cmap_div.set_bad(color="#d9d9d9")
    norm_vis = mcolors.TwoSlopeNorm(vmin=cmin_vis, vcenter=0.0, vmax=cmax_vis)
    norm_txt = mcolors.TwoSlopeNorm(vmin=cmin_txt, vcenter=0.0, vmax=cmax_txt)

    # -- figure setup (manual positioning for top-alignment) ------------------
    # Layout: vision heatmap (left, full height), text heatmap (right, top-
    # aligned, shorter), legend cells + two colorbars below text heatmap.
    ...
    im_left = ax_left.imshow(masked_vis, cmap=cmap_div, norm=norm_vis, aspect="equal")
    ...
    ax_left.set_title(r"\textbf{" + f"Vision: {vision_display}" + "}", fontsize=9, pad=6)

    # -- right heatmap (text, labels on the right) ----------------------------
    masked_txt = np.ma.masked_invalid(z_txt)
    im_right = ax_right.imshow(masked_txt, cmap=cmap_div, norm=norm_txt, aspect="equal")
    ...
    ax_right.set_title(r"\textbf{" + f"Text: {text_display_name}" + "}", fontsize=9, pad=6)

    # -- legend cells + colorbars (below text heatmap) ------------------------
    ...
    legend_h = (below_top - below_bot) * 0.3
    legend_y = (below_top + below_bot) / 2.0 - legend_h * 0.8
    ax_legend = fig.add_axes([x_txt, legend_y, w_txt, legend_h])
    ax_legend.set_axis_off()

    legend_items = [
        ("Harmful",     cmin_vis),
        ("Helpful",     max(cmax_vis, cmax_txt) * 0.3),
        ("Strong gain", max(cmax_vis, cmax_txt)),
    ]
    ...
    for idx, (label, value) in enumerate(legend_items):
        ...
        # use vision norm for legend color samples
        color = cmap_div(norm_vis(value))
        rect = plt.Rectangle(
    ...
    # two horizontal colorbars stacked
    cbar_h = 0.02
    cbar_gap = 0.04
    cbar_y_vis = below_bot + cbar_gap + cbar_h + cbar_gap
    cbar_y_txt = below_bot + 0.02

    cbar_ax_vis = fig.add_axes([x_txt, cbar_y_vis, w_txt, cbar_h])
    cbar_vis = fig.colorbar(im_left, cax=cbar_ax_vis, orientation="horizontal")
    cbar_vis.set_label("Vision: Top-1 improvement vs naive PTQ", fontsize=9)
    cbar_vis.ax.tick_params(labelsize=9)

    cbar_ax_txt = fig.add_axes([x_txt, cbar_y_txt, w_txt, cbar_h])
    cbar_txt = fig.colorbar(im_right, cax=cbar_ax_txt, orientation="horizontal")
    cbar_txt.set_label("Text: Top-1 improvement vs naive PTQ", fontsize=9)
    cbar_txt.ax.tick_params(labelsize=9)
    ...
    out_dir = os.path.join(
        ...
        "qv_transfer_heatmap_best_alpha_independent_cscale",
