"""Radar plot: FT+PTQ vs QV Patching+PTQ vs QAT+PTQ (mixed modalities)

Two radar plots side-by-side: one vision model (timm supervised) and one
text model (automodel for sequence classification).  Each radar has three webs:
...
def _qat_ptq_path(eval_root, model_dir, dataset, seed, optim_frag, qat_frag, ptq_frag):
    return os.path.join(
